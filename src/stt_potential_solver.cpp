...
static void system_inside(double r, double* y, double* yprime,double alpha)
{
    ...
    double A = A_phi(y[4],alpha);
    double V = V_phi(y[4],alpha);
    double dV = dV_phi(y[4],alpha);

    double eps = e_at_p(y[3]);


    yprime[0] = 4*PI*pow(A,4)*pow(r,2.0)*eps + 0.5*r*(r-2*y[0])*pow(y[2],2) + 0.25*V*pow(r,2);
   
    yprime[1] = 2.0*(y[0] + 4.0*PI*pow(A,4)*pow(r,3.0)*y[3] - 0.25*V*pow(r,3.0))/( r*(r-2.0*y[0])) + r*pow(y[2],2);

    yprime[2] = 4*PI*pow(A,4)*r*(alpha_phi*(eps-3.0*y[3]) + r*(eps-y[3])*y[2] )/(r-2.0*y[0]) - 2*y[2]*(1 - y[0]/r)/(r - 2.0*y[0]) + (0.5*V*y[2]*pow(r,2.0) + 0.25*dV*r)/(r - 2.0*y[0]);

    yprime[3] = -(eps + y[3])*((y[0] + 4.0*PI*pow(A,4)*pow(r,3.0)*y[3] - 0.25*V*pow(r,3.0))/(r*(r-2.0*y[0])) + 0.5*r*pow(y[2],2) + alpha_phi*y[2]);
...
double function_to_minimize(double args[2])
{
  ...
  int istate_obj = 1;
	...
	double met_lamda_c_obj,omega_c_obj,pr_c_obj;
  ...
  while(t_obj<r_vec_obj[N-1]){
  	...
  	lsoda.lsoda_update(system_outside, neq, y_obj, yout_obj, &t_obj, tout_obj, &istate_obj, coupling,1e-06,1e-20);
        res_la_obj.push_back(yout_obj[1]);
        res_ph_obj.push_back(yout_obj[2]);
        res_om_obj.push_back(yout_obj[3]);
        res_pr_obj.push_back(yout_obj[4]);
...
return pow(res_ph_obj[idx_obj-3],2)+pow(res_phi_obj[idx_obj-3],2);
}
...
int main(int argc, char *argv[])
{
  ...
  std::vector<double> dphdr,dphidr;

  dphdr = gradient(res_ph,dr);
  //dphidr = gradient(res_phi,dr);
  //Mass = dphdr[N-3]*pow(r_vec[N-10],2)*exp(res_ph[N-10])/2;

          
  Mass = dphdr[N-3]*pow(r_vec[N-3],2)/(2*(1+r_vec[N-3]*dphdr[N-3]));
  scalar_charge = res_om[N-3]*pow(r_vec[N-3],2);
 ...
 switch(print_option){
    ...
    printf("\n");
    printf("  %6.5e  M             (M_sun)\n",Mass);
    printf("  %6.5e  R             (km)\n",r_vec[idxlast]*A_phi(res_phi[idxlast],coupling)*Length/pow(10,5));
