...
public:
    ...
    size_t idamax1(const vector<double> &dx, const size_t n, const size_t offset = 0);

    void dscal1(const double da, vector<double> &dx, const size_t n, const size_t offset = 0);
    ...
    void prja(const size_t neq, vector<double> &y, theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM  f, double data);

    void lsoda(theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM  f, const size_t neq, vector<double> &y, double *t,
        double tout, int itask, int *istate, int iopt, int jt, array<int, 7> &iworks,
        array<double, 4> &rworks, double data);

    void correction(const size_t neq, vector<double> &y, theoryType* model,typename TYPES<theoryType>::ODE_SYSTEM  f,
        size_t *corflag, double pnorm, double *del, double *delp, double *told,
        size_t *ncf, double *rh, size_t *m, double data);

    void stoda(const size_t neq, vector<double> &y, theoryType* model,typename TYPES<theoryType>::ODE_SYSTEM  f, double data);

    // We call this function in VoxelPools::
    void lsoda_update(theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM  f, const size_t neq, vector<double> &y,
        std::vector<double> &yout, double *t, const double tout, int *istate,
        double data, double rtol, double atol  // Tolerance
    );
...
template<typename theoryType>
bool LSODA<theoryType>::abs_compare(double a, double b)
{
...
template<typename theoryType>
size_t LSODA<theoryType>::idamax1(const vector<double> &dx, const size_t n, const size_t offset)
{
    size_t v = 0, vmax = 0;
    size_t idmax = 1;
...
*/
template<typename theoryType>
void LSODA<theoryType>::dscal1(
    const double da, vector<double> &dx, const size_t n, const size_t offset)
{
...
template<typename theoryType>
double LSODA<theoryType>::ddot1(const vector<double> &a, const vector<double> &b, const size_t n,
...
template<typename theoryType>
void LSODA<theoryType>::daxpy1(const double da, const vector<double> &dx, vector<double> &dy,
...
template<typename theoryType>
void LSODA<theoryType>::dgesl(const vector<vector<double>> &a, const size_t n, vector<int> &ipvt,
    vector<double> &b, const size_t job)
{
    size_t k, j;
    double t;
...
template<typename theoryType>
void LSODA<theoryType>::terminate(int *istate)
{
    if(illin == 5)
...
template<typename theoryType>
void LSODA<theoryType>::terminate2(vector<double> &y, double *t)
{
    for(size_t i = 1; i <= n; i++)
        y[i] = yh_[1][i];
    *t    = tn_;
...
template<typename theoryType>
void LSODA<theoryType>::successreturn(
    vector<double> &y, double *t, int itask, int ihit, double tcrit, int *istate)
{
    for(size_t i = 1; i <= n; i++)
        y[i] = yh_[1][i];
...
*/
template<typename theoryType>
void LSODA<theoryType>::lsoda(theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM f, const size_t neq, vector<double> &y, double *t,
        double tout, int itask, int *istate, int iopt, int jt, array<int, 7> &iworks,
        array<double, 4> &rworks, double data)
...
template<typename theoryType>
void LSODA<theoryType>::stoda(
    const size_t neq, vector<double> &y, theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM f, double data)
{
    assert(neq + 1 == y.size());
    ...
    */
    while(1) {
        while(1) {
            ...
            correction(
                neq, y, model, f, &corflag, pnorm, &del, &delp, &told, &ncf, &rh, &m, data);
            if(corflag == 0)
                break;
            if(corflag == 1) {
...
template<typename theoryType>
void LSODA<theoryType>::ewset(const vector<double> &ycur)
{
    switch(itol_) {
        case 1:
            for(size_t i = 1; i <= n; i++)
...
*/
template<typename theoryType>
void LSODA<theoryType>::intdy(double t, int k, vector<double> &dky, int *iflag)
{
    int ic, jp1 = 0;
    double c, r, s, tp;
...
template<typename theoryType>
void LSODA<theoryType>::cfode(int meth_)
{
    int i, nq, nqm1, nqp1;
...
template<typename theoryType>
void LSODA<theoryType>::scaleh(double *rh, double *pdh)
{
    double r;
    /*
...
*/
template<typename theoryType>
double LSODA<theoryType>::vmnorm(const size_t n, const vector<double> &v, const vector<double> &w)
{
    double vm = 0.;
    for(size_t i = 1; i <= n; i++)
...
template<typename theoryType>
double LSODA<theoryType>::fnorm(int n, const vector<vector<double>> &a, const vector<double> &w)
...
*/
template<typename theoryType>
void LSODA<theoryType>::correction(const size_t neq, vector<double> &y, theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM f,
...
template <typename theoryType>
void LSODA<theoryType>::corfailure(double *told, double *rh, size_t *ncf, size_t *corflag)
{
    ncf++;
    rmax = 2.;
    tn_  = *told;
...
*/
template<typename theoryType>
void LSODA<theoryType>::solsy(vector<double> &y)
{
    iersl = 0;
    if(miter != 2) {
...
template<typename theoryType>
void LSODA<theoryType>::methodswitch(double dsm, double pnorm, double *pdh, double *rh)
{
...
*/
template<typename theoryType>
void LSODA<theoryType>::endstoda()
{
...
*/
template<typename theoryType>
void LSODA<theoryType>::orderswitch(
    double *rhup, double dsm, double *pdh, double *rh, size_t *orderflag)
{
    size_t newq = 0;
...
template<typename theoryType>
void LSODA<theoryType>::resetcoeff()
/*
...
template<typename theoryType>
void LSODA<theoryType>::_freevectors(void)
{
...
/* ----------------------------------------------------------------------------*/
template<typename theoryType>
void LSODA<theoryType>::lsoda_update(theoryType* model, typename TYPES<theoryType>::ODE_SYSTEM f, const size_t neq, vector<double> &y,
    ...
    double rtol, double atol)
{
    ...
    lsoda(model, f, neq, yout, t, tout, itask, istate, iopt, jt, iworks, rworks, data);
}
